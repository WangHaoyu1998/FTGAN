...
class VanGanCwru(object):
    ...
    def setup(self):
        ...
        cwru_dataset = CWRUDataset(rootpath=self.args.data_path,
                                   aim_data=self.aim_data)
        self.train_loader = DataLoader(cwru_dataset, batch_size=self.args.batch_size,
        ...
        for epoch in range(self.args.epochs):
            ...
            if epoch % 10 == 0:
                with torch.no_grad():
                    ...
                    save_dir = './training process/generation/cwru/VanGAN/' + self.aim_data
                    self.gen.eval()
    ...
    def train(self):
        ...
        for epoch in range(self.args.epochs):
            ...
            if epoch % 10 == 0:
                with torch.no_grad():
                    from process_saving import ProcessSaving
                    save_dir = './training process/generation/cwru/DCGAN/' + self.aim_data
                    self.gen.eval()
                    real_sig = next(iter(self.train_loader))[0]
                    gen_sig = self.gen(fixed_z)
...
class FtGanCwru(object):
    def __init__(self, args, aim_data):
        self.args = args
        self.aim_data = aim_data
        self.device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    ...
    def train(self):
        ...
        for epoch in range(self.args.epochs):
            ...
            if epoch % 10 == 0:
                from process_saving import ProcessSaving
                save_dir = './training process/generation/cwru/FTGAN/' + self.aim_data
                self.gen.eval()
                real_sig = next(iter(self.train_loader))[0]
                gen_sig = self.gen(fixed_z)
...
if __name__ == '__main__':
    ...
    # dc = DcGanCwru(args=parse_args(), aim_data="0.007-OuterRace6")
    # dc.setup()
    # dc.train()
    ...
    #     dc = DcGanCwru(args=parse_args(), aim_data=aim_data)
    #     dc.setup()
    #     dc.train()
    #     torch.cuda.empty_cache()
    #     ft = FtGanCwru(args=parse_args(), aim_data=aim_data)
    #     ft.setup()
    #     ft.train()
    #     torch.cuda.empty_cache()
