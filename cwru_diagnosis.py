...
def parse_args():
    ...
    parser.add_argument('-data_path', type=str, default='./Data/CaseWesternReserveUniversity',
                        help='the directory of the data')
    parser.add_argument('-model_path', type=str, default='./training process/generation/cwru',
                        help='the directory of generator model')
    parser.add_argument('-batch_size', type=int, default=64,
                        help='batch size of the training process')
    parser.add_argument('-cnn_lr', type=float, default=2e-3,
...
class CWRU_Diag(object):
    def __init__(self, args, imb_ratio=1, gen_model='FTGAN', cnn_model='lenet', purpose='trts'):
        self.args = args
    ...
    def setup(self):
        from cwru_dataset_load import CWRU_MixedDataset
        # make dataloader
        self.train_set = CWRU_MixedDataset(rootpath=self.args.data_path, model_path=self.args.model_path,
        ...
        self.test_set = CWRU_MixedDataset(rootpath=self.args.data_path, model_path=self.args.model_path,
        ...
        self.diag_model = getattr(modules, self.cnn_model)\
            (in_channel=1, out_channel=10).to(self.device)
        # optimizer
    ...
    def train(self):
        ...
        dr = DiagnosisResults(save_dir='./training process/diagnosis/cwru/'
...
if __name__ == '__main__':
    ...
    for gen_model in gen_models:
        for cnn_model in cnn_models:
            for purpose in purposes:
                torch.cuda.empty_cache()
                diag = CWRU_Diag(args, gen_model=gen_model,
    ...
    # args = parse_args()
    # torch.cuda.empty_cache()
    # diag = CWRU_Diag(args, gen_model=gen_models[2], imb_ratio=imb_ratios[3],
